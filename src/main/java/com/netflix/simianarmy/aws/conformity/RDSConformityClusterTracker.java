...
public class RDSConformityClusterTracker implements ConformityClusterTracker {
	 ...
	 *
	 */
	public RDSConformityClusterTracker(String[] aap) {
		HikariDataSource dataSource = new HikariDataSource();
		dataSource.setDriverClassName(aap[0]);
		dataSource.setJdbcUrl(aap[3]);
		dataSource.setUsername(aap[1]);
		dataSource.setPassword(aap[2]);
	...
	public void addOrUpdate(Cluster cluster) {
		...
		LOGGER.debug(String.format("Saving cluster %s to RDB table %s in region %s", cluster.getName(),
				cluster.getRegion(), table));
		...
		try {
			...
			LOGGER.error("ERROR generating conformities JSON when saving cluster " + cluster.getName() + ", "
					+ cluster.getRegion(), e);
			return;
		}
		...
		if (orig == null) {
			...
			this.jdbcTemplate.update(sb.toString(), value(map.get(Cluster.CLUSTER)), value(map.get(Cluster.REGION)),
					emailValue(map.get(Cluster.OWNER_EMAIL)), value(map.get(Cluster.IS_CONFORMING)),
					value(map.get(Cluster.IS_OPTEDOUT)), value(cluster.getUpdateTime()),
					value(map.get(Cluster.EXCLUDED_RULES)), value(conformityJson),
			...
			this.jdbcTemplate.update(sb.toString(), emailValue(map.get(Cluster.OWNER_EMAIL)),
					value(map.get(Cluster.IS_CONFORMING)), value(map.get(Cluster.IS_OPTEDOUT)),
					value(cluster.getUpdateTime()), value(map.get(Cluster.EXCLUDED_RULES)), value(conformityJson),
					value(map.get(Cluster.CONFORMITY_RULES)), value(cluster.getName()), value(cluster.getRegion()));
		}
	...
	/**
	 * Gets the clusters for a list of regions. If the regions parameter is
	 * empty, returns the clusters for all regions.
	 */
	@Override
	...
	public Cluster getCluster(String clusterName, String region) {
		...
		List<Cluster> clusters = jdbcTemplate.query(query.toString(), new String[] { clusterName, region },
	...
	private List<Cluster> getClusters(Boolean conforming, String... regions) {
		...
		LOGGER.info(String.format("Query to retrieve clusters for regions %s is '%s'", StringUtils.join(regions, "','"),
				query.toString()));
		...
		LOGGER.info(String.format("Retrieved %d clusters from RDS DB in table %s and regions %s", clusters.size(),
				table, StringUtils.join(regions, "','")));
		return clusters;
	}
	...
	public void init() {
		try {
			...
			String sql = String.format(
					"create table if not exists %s (" + " %s varchar(255)," + " %s varchar(25)," + " %s varchar(255),"
							+ " %s varchar(10)," + " %s varchar(10)," + " %s BIGINT," + " %s varchar(4096),"
							+ " %s varchar(4096)," + " %s varchar(4096) )",
					table, Cluster.CLUSTER, Cluster.REGION, Cluster.OWNER_EMAIL, Cluster.IS_CONFORMING,
					Cluster.IS_OPTEDOUT, Cluster.UPDATE_TIMESTAMP, Cluster.EXCLUDED_RULES, "conformities",
